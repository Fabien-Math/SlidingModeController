...
def load_sequences(etas_seq, nus_seq):
    global etas, nus
    etas = etas_seq
    nus = nus_seq
...
def display():
    ...
    if follow_robot and etas is not None:
        tf = etas[frame_index % len(etas)]
        pos = tf[:3]
    ...
    if etas is not None:
        draw_robot(etas[frame_index % len(etas)])
    ...
    if etas is not None:
        ...
        draw_text(x_pos, 50, tf_str)

        nu = nus[frame_index % len(etas)]
        nu_str = f"Vel: ({nu[0]:.2f}, {nu[1]:.2f}, {nu[2]:.2f}) vel RPY: ({nu[3]:.2f}, {nu[4]:.2f}, {nu[5]:.2f}) norm {np.linalg.norm(nu):.2f}"
        text_width = len(nu_str) * 9  # approx width per char in pixels
        x_pos = 600 - text_width // 2
        draw_text(x_pos, 10, nu_str)
    glEnable(GL_LIGHTING)
...
def run_viewer(etas, nus):
    load_sequences(etas, nus)
    glutInit()
